...
public class HouseholdTemplateCreator {
  ...
  public static String createOutcomeMessage(EventType eventType, Map<String, Object> root) {
    ...
    try {
      ...
      Template temp = configuration.getTemplate("household/" + eventType.name() + "-event.ftl");
      try (StringWriter out = new StringWriter(); StringWriter outcomeEventMessage = new StringWriter()) {
        ...
        Template outcomeMessageTemplate = configuration.getTemplate("rm-household-event.ftl");
